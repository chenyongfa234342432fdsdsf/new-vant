...
async function transformer(code, id, reactBabelPlugin, remarkOpts) {
  ...
  const { demos, slugs, frontmatter, value, title, updatedTime } = await parser(code, id, remarkOpts);
  ...
  ${demos.map((demo) => {
    var _a;
    const request = `${slash(id)}.${demo.name}.${Buffer.from(id).toString("base64")}.${((_a = demo.props) == null ? void 0 : _a.lang) || "jsx"}`;
    demo.id = request;
  ...
  return {
    ...
    resolveId(_id) {
      ...
      if (mat && mat.length > 2) {
        ...
        const idPath = id.startsWith(sourceId) ? id : path.join(CWD, id.substring(1));
        return idPath;
      }
    },
    load(_id) {
      ...
      if (mat && mat.length >= 2) {
        ...
        if (demo.filePath) {
          return {
            code: `import ${demo.name}, { ${FILE_PATH_EXPORT_NAME} } from '${slash2(demo.filePath)}';
 ...
 export const ${FILE_PATH_EXPORT_NAME} = ${JSON.stringify(idSource)}`;
    ...
    async transform(code, id) {
      if (filter(id)) {
        ...
        cache.set(slash2(id), demos);
        demos.forEach((demo) => {
          if (demo.filePath) {
            importedIdSet.set(slash2(demo.filePath), id);
          }
        });
        return { code: content };
      }
    ...
    async handleHotUpdate(ctx) {
      if (filter(ctx.file)) {
        ...
        cache.set(slash2(ctx.file), demos);
        const updateModules = [];
        demos.forEach((demo) => {
          if (demo.filePath)
            return;
